...
一套基于 RuoYi 3.9.2 的前后端分离仓储管理系统，覆盖仓库、库区、库位、物料、实时库存、库存流水、入库单、出库单和运营看板。

- 后端：Spring Boot 4、Spring Security、JWT、MyBatis、MySQL、Redis
- 前端：Vue 3、Vite、Element Plus、Pinia
- 核心规则：单据草稿控制、入出库事务过账、库存不足拦截、并发防超卖、全链路库存流水

本地启动、初始化账号、模块说明和生产注意事项请阅读 [README-WMS.md](README-WMS.md)。数据库依赖可通过根目录的 `docker-compose.yml` 启动。

> 本项目保留 RuoYi 的 MIT 许可证与原始版权信息；以下为上游框架说明。
